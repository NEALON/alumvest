class InvestmentTermsController < ApplicationController
  ...
  def create
    @campaign = Alumvest::CampaignBase.find(params[:alumvest_investment_term_base][:campaign_id])
    authorize! :manage, @campaign
    @investment_term = Alumvest::InvestmentTermBase.create(add_images_if_test!(params[:alumvest_investment_term_base]))
