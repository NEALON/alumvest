...
module Veritax::Order::Events
  ...
  included do
    ...
    def fire_order_completed
      Bus::Event::VeritaxOrder::Completed.create(:veritax_order => self, :investor => investor)
      InvestorMailer.veritax_order_status_updated(investor.user.email).deliver
    end

    def fire_order_canceled
      Bus::Event::VeritaxOrder::Canceled.create(:veritax_order => self, :investor => investor)
      InvestorMailer.veritax_order_status_updated(investor.user.email).deliver
    end

    def fire_order_approved
      Bus::Event::VeritaxOrder::Approved.create(:veritax_order => self, :investor => investor)
      InvestorMailer.accredited_investor_approved(investor.user.email).deliver
      investor.approve!
    end

    def fire_order_rejected
      Bus::Event::VeritaxOrder::Rejected.create(:veritax_order => self, :investor => investor)
