...
module Veritax::OrderEvents
  ...
  included do
    def fire_order_errored
      ...
      #InvestorMailer.veritax_order_status_updated(investor).deliver
    end
    ...
    def fire_order_approved
      ...
      InvestorMailer.accredited_investor_approved(investor).deliver
      investor.approve!
    end

    def fire_order_rejected
      Bus::Event::VeritaxOrder::Rejected.create(:veritax_order_id => self.id, :investor => investor)
      InvestorMailer.accredited_investor_rejected(investor).deliver
    end
  end
end
