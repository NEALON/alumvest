module RailsthemesHelper
  ...
  def grouped_wireframes
    ...
    wireframes.sort.group_by { |name| name.gsub(/-.*/, "") }
  end
  ...
  def example_tabs(partial_path, opts = {})
    ...
    locals = {
      ...
      :rel => rel,
      :link_opts => link_opts,
      :preview => render(:partial => File.join('railsthemes', 'previews', partial_path)),
      :tag => opts[:tag],
  ...
  def source_code(partial_path, render_view = true)
    ...
    path     = File.join('railsthemes', 'previews', File.dirname(partial_path))
