...
describe 'users investor income verification', :type => :feature do
  ...
  it 'copies existing field values' do
    expect(page.find_field('veritax_order_order_ssn').value).to eq(@user.ssn || '')
    expect(page.find_field('veritax_order_order_first_name').value).to eq(@user.first_name)
    expect(page.find_field('veritax_order_order_last_name').value).to eq(@user.last_name || '')
    expect(page.find_field('veritax_order_order_address').value).to eq(@user.address || '')
    expect(page.find_field('veritax_order_order_city').value).to eq(@user.city || '')
    expect(page.find_field('veritax_order_order_state').value).to eq(@user.state || '')
    expect(page.find_field('veritax_order_order_zip_code').value).to eq(@user.zipcode || '')
    expect(page.find_field('veritax_order_order_email').value).to eq(@user.email || '')
  end
